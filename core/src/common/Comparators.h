...
// TODO(jiawen): move to a namespace.
...
public:
    ...
    static bool pairFirstElementLess( const std::pair< T0, T1 >& a,
        const std::pair< T0, T1 >& b );

    // Compares only the second element of a pair.
    template< typename T0, typename T1 >
    static bool pairSecondElementLess( const std::pair< T0, T1 >& a,
        const std::pair< T0, T1 >& b );

    // Returns true if x.second < y.second.
    // Useful for sorting array indices based on distance.
    static bool indexAndDistanceLess( const std::pair< int, float >& a,
        const std::pair< int, float >& b );

    // Returns true if x.second > y.second.
    // Useful for sorting array indices based on distance.
    static bool indexAndDistanceGreater( const std::pair< int, float >& a,
        const std::pair< int, float >& b );

    static bool vector2iLexigraphicLess( const Vector2i& a,
        const Vector2i& b );

    static bool vector3iLexigraphicLess( const Vector3i& a,
        const Vector3i& b );

    static bool vector4iLexigraphicLess( const Vector4i& a,
        const Vector4i& b );
};
