...
Array1DWriteView< T > Array3DWriteView< T >::xySlice( const Vector2i& xy )
{
    return Array1DWriteView< T >(
        elementPointer( { xy.x, xy.y, 0 } ), this->size().z );
}
...
{
    return Array1DWriteView< T >(
        elementPointer( { 0, yz.x, yz.y } ), this->size().x );
}
...
{
    return Array1DWriteView< T >(
        elementPointer( { xz.x, 0, xz.y } ), this->size().y );
}
...
Array2DWriteView< T > Array3DWriteView< T >::xSlice( int x )
{
    return Array2DWriteView< T >(
        elementPointer( { x, 0, 0 } ), this->size().yz, this->stride().yz );
}
...
{
    return Array2DWriteView< T >(
        slicePointer( z ), this->size().xy, this->stride().xy );
}
